...
- (void)resetPlayerToPlayerLayer {
    self.player.playerLayer.player = _player.player;
        NSLog(@"应用进入前台");
...
- (void)startWithTime:(NSTimeInterval )time{
    //转换成time
    CMTime currentTime = CMTimeMakeWithSeconds(time, 1);
    //切换开始播放时间
    [self.player.playerItme seekToTime:currentTime completionHandler:^(BOOL finished) {
        
    }];
    //待写。  刚进入的进度条
//    [self updateVideoProgressSlider:time];
}
...
- (void)setUI {
    ...
    [_videoSlider mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(self.currentTimeLabel.mas_right).with.offset(2);
        make.right.equalTo(self.totalTimeLabel.mas_left).with.offset(-2);
...
//    NSLog(@"当前时间：%ld分：%ld秒",(long)proMin,(long)proSec);
}
#pragma mark - Events
// 是否显示控件
...
- (UISlider *)videoSlider {
    if (!_videoSlider) {
        ...
        _videoSlider.minimumTrackTintColor = YellowColor;
