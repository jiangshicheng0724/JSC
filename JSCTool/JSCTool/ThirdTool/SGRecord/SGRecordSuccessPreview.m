...
-(void)layoutSubviews{
    ...
    [UIView animateWithDuration:0.25 animations:^{
        self->_cancelButton.bounds = CGRectMake(0, 0, _width, _width);
        self->_cancelButton.center = CGPointMake(self.bounds.size.width / 4, self.bounds.size.height -_distance - _width/2);
        self->_sendButton.bounds = CGRectMake(0, 0, _width, _width);
        self->_sendButton.center = CGPointMake(self.bounds.size.width / 4 * 3, self.bounds.size.height - _distance - _width/2);
    }];
}
#if __IPHONE_OS_VERSION_MAX_ALLOWED > __IPHONE_8_4
- (void)replay{
