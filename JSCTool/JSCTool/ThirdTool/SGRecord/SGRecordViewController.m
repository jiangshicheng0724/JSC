...
- (void)stopRecord:(BOOL)isSuccess{
    ...
    [self.recordManger stopCaptureWithStatus:isSuccess handler:^(UIImage *movieImage,NSString *filePath) {
        DLog(@"第一帧:image:%@",movieImage);
        STRONGSELF
        dispatch_async(dispatch_get_main_queue(), ^{
            [strongSelf.recordManger shutdown];
            [strongSelf.preview setImage:movieImage videoPath:filePath captureVideoOrientation:[[SGMotionManager sharedManager] currentVideoOrientation]];
        });
    }];
}
#pragma mark -
...
- (void)sendWithImage:(UIImage *)image videoPath:(NSString *)videoPath{
    DLog(@"发送");
    if (self.recordViewBackBlock) {
        
        self.recordViewBackBlock(videoPath, image);
    }
...
- (void)caculateTime{
    ...
    DLog(@"计时器:_timeInterval:%f",_timeInterval);
...
- (void)toucheUpInsideOrOutSide:(UIButton *)button{
    DLog(@"抬起按钮:__timeInterval==:%f",_timeInterval);
    [self removeTimer];
...
-(void)motionManagerDeviceOrientation:(UIDeviceOrientation)deviceOrientation{
    ...
    [UIView animateWithDuration:0.25 animations:^{
        self->_exitButton.transform = CGAffineTransformRotate(CGAffineTransformIdentity, angle);
        self->_switchButton.transform = CGAffineTransformRotate(CGAffineTransformIdentity, angle);
    }];
    _lastDeviceOrientation = deviceOrientation;
    DLog(@"deviceOrientation:%ld",(long)deviceOrientation);
}
- (void)alart{
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        if (self->_alartLabel) {
            [UIView animateWithDuration:0.25 animations:^{
                self->_alartLabel.alpha = 0;
            } completion:^(BOOL finished) {
                [self->_alartLabel removeFromSuperview];
                self->_alartLabel = nil;
            }];
        }
    });
}
